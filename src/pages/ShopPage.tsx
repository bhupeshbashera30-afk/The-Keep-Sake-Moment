...
export function ShopPage() {
  ...
  const handleAdd = (product: Product) => {
    addItem({ id: product.id, name: product.name, price: product.price, image_url: product.image_url, category: product.category })
    setAdded(product.id)
          ...
          <ScrollReveal>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {products.map(product => (
                <article
                  key={product.id}
                  className="card-lift group flex flex-col rounded-3xl border border-burgundy-100 bg-white p-6 shadow-soft"
                >
